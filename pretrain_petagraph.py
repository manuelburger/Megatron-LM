...
from megatron.core.datasets.petagraph_dataset import PetaGraphStreamDataset
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    if data_path.is_dir():
        # Assume it's local mock data
        mock_data = True

        data_url_list = [str(data_path / f"sample_{i}.unitigs.fa.zst") for i in range(5)]
        train_list = data_url_list[:-2]
        val_list = data_url_list[-2:-1]
        test_list = data_url_list[-1:]
    else:
        # It's a text file with contigs
        # unitigs sources to load from AWS
        mock_data = False

        # Load files with lines
        if "unitig" in data_path.name:
            load_type = "unitig"
            url_format = "s3://logan-pub/u/{accession}/{accession}.unitigs.fa.zst"
        elif "contig" in data_path.name:
            load_type = "contig"
        ...
        num_val_files = 100_000
        num_test_files = 100_000
        train_list = all_files[:-num_val_files - num_test_files]
        val_list = all_files[-num_val_files - num_test_files:-num_test_files]
        test_list = all_files[-num_test_files:]
        print_rank_0(f"Train: {len(train_list)} Val: {len(val_list)} Test: {len(test_list)}")
